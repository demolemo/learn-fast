...
def parse_args():
    ...
    parser.add_argument('--file-length-hrs', type=float, default=3.2)
    parser.add_argument('--cold-start', type=bool, default=True, action=argparse.BooleanOptionalAction)
    parser.add_argument('--testing', type=bool, default=True, action=argparse.BooleanOptionalAction)
    return parser.parse_args()


def chunk_audio(file_name, file_length_hrs):
    file_size_mb = os.path.getsize(file_name) / (1024 * 1024)
    number_of_chunks = int(file_size_mb // CHUNK_SIZE_MB) + 1
    total_len_ms = int(file_length_hrs * 60 * 60 * 1000) + 1000
...
def main():
    args = parse_args()
    if args.cold_start:
        chunk_audio(args.file_name, args.file_length_hrs)
        model = load_model()
